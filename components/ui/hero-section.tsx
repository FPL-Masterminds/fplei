...
const HeroSection = React.forwardRef<HTMLDivElement, HeroSectionProps>(
  (
    ...
    return (
      <div className={cn("relative bg-white", className)} ref={ref} {...props}>
        <section className="relative max-w-full mx-auto z-1">
          <RetroGrid {...gridOptions} />
          <div className="max-w-screen-xl z-10 mx-auto px-4 py-28 gap-12 md:px-8">
            <div className="space-y-5 max-w-3xl leading-0 lg:leading-5 mx-auto text-center">
              <h1 className="text-sm text-[#2E0032] group font-inter mx-auto px-5 py-2 bg-gradient-to-tr from-zinc-300/20 via-gray-400/20 to-transparent border-[2px] border-black/5 rounded-3xl w-fit">
                {title}
                <ChevronRight className="inline w-4 h-4 ml-2 group-hover:translate-x-1 duration-300" />
              </h1>
              <h2 className="text-4xl tracking-tighter font-inter md:text-6xl text-[#2E0032]">
                {subtitle.regular}
                <span className="text-[#00FF87]">{subtitle.gradient}</span> with data-driven insights
              </h2>
              <p className="max-w-2xl mx-auto text-[#2E0032]">
                {description}
              </p>
